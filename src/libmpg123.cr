...
lib LibMP
  ...
  fun feed = mpg123_feed(MP_Handle*, CUChar*, Size) : CInt
  fun open_feed = mpg123_open_feed(MP_Handle*) : CInt
...
module Libmpg123
  class Mp
    ...
    def decode(in_buf, in_size, out_buf, out_size, done)
      LibMP.decode(@handle, in_buf, in_size, out_buf, out_size, done)
    end

    def meta_free
      LibMP.meta_free(@handle)
    end
